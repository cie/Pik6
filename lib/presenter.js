...
// Update the note content each time the slides change. If there is content to display,
// add the class `pik-active` to the notes container.
var notes_container = $('Pik-notes'),
    notes_content   = $('Pik-notes-content');
window.addEvent('pikSlide', function(activated){
	var slide = $$('iframe')[0].contentWindow.pik.slides[activated];
	if(slide){
