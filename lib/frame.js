...
// Parse a state url. I know this is uber ugly but that's URL parsing for you
var parseStateUrl = function(){
	...
	if(typeof state_url !== 'undefined'){
		...
		for(var i = 0, l = split_state_url.length; i < l; i++){
			...
			// "hidden" parameter
			if(split_state_url[i] === split_state_url[0] && split_state_url[i] === 'hide'){
			...
			// "slide" parameter. Also set hide to false
...
// function should always be used to set state, except when dealing with updates recieved from the worker. The
// function expects a state object. If the object contains changes to the local state, the changes are applied,
// the woker is notified and the url is changed.
...
// If we don't have a worker, parse the state url and set the local state accordingly. Otherwise
// the worker takes care of the initial state
if(!supports_worker){
