...
// When a pikFile event fires, update the frame's iframes with the new presentation
...
parseStateUrl = function(){
	...
	if(typeof state_url !== 'undefined'){
		...
		for(var i = 0, l = split_state_url.length; i < l; i++){
			...
			// Remember to substract -1 from the url parameter!
			else if(split_state_url[i] === split_state_url[0] &&
				    !isNaN(parseInt(split_state_url[i], 10))){
				var slide_number = parseInt(split_state_url[i], 10) - 1;
