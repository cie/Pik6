...
var clearWorkerInit = function(){
	workerInitalized = true;
	initalUpdates(getCurrent(), false);
}

/* Process incoming worker messages */
var handleMessage = (function(){
	return (!worker) ? null : function(evt){
console.log('Empfange', (evt.data) ? evt.data.file : null, (evt.data) ? evt.data.hash : '/');
		/* Set the status according to the message recieved */
		if(evt.data && typeof evt.data.hash !== 'undefined' && evt.data.hash !== getCurrent()){
			setCurrent(evt.data.hash, false, 'handleMessage');
		}
		if(evt.data && typeof evt.data.file !== 'undefined' && evt.data.file !== iframes[0].contentWindow.location.href.split('#')[0]){
...
console.log('Sende', (message) ? message.file : null, (message) ? message.hash : '/');
...
var setCurrent = function(x, propagate, from){
	location.hash = x;
console.log('Current gesetzt auf', x, from);
	if(worker && propagate){
		postMessage();
	}
}
...
/* Fire the activate, deactivate and change events in the iframes. The function's arguments
...
/* Watch the window for hash changes and delegate the changes to the iframes */
...
console.log('Initalisiere...')
		if(worker && !workerInitalized){
			workerInit();
		}
		else {
			initalUpdates();
			setCurrent(getCurrent(), true, 'init');
		}
