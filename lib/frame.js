...
pik.init = function(pik_base_url){
	...
	// Changing the `<title>` throws a `NO_MODIFICATION_ALLOWED_ERR` DOMCoreException in
	// the default browser for Android 2.3
	try {
...
var reInit = function(){
	// New slide set
	slides = iframes[0].contentWindow.pik.slides;
	// New title. Remember that changing the `<title>` throws a
	// `NO_MODIFICATION_ALLOWED_ERR` DOMCoreException in the default browser for Android 2.3
	var frame_title = iframes[0].contentWindow.document.getElements('title').get('html');
	try {
		$$('title').set('html', frame_title);
	} catch(e){}
